...
<div class="paper-toolbar">
  <a href="https://arxiv.org/abs/2509.05165" class="toolbar-btn" target="_blank">📄 arXiv: 2509.05165v2</a>
  <a href="https://arxiv.org/pdf/2509.05165.pdf" class="toolbar-btn" target="_blank">📥 PDF</a>
  <button class="toolbar-btn favorite-btn" data-arxiv-id="2509.05165v2" data-paper-url="__CURRENT_PAGE__" onclick="toggleFavorite(this, '2509.05165v2', 'KVCompose: Efficient Structured KV Cache Compression with Composite Tokens')" title="添加到收藏夹">☆ 收藏</button>
...
1. 长文本LLM推理中，KV缓存随上下文增长迅速，成为内存瓶颈，现有压缩方法存在启发式规则僵化、破坏张量结构或依赖专用计算内核等问题。
2. KVCompose通过注意力机制引导，自适应地为每一层选择重要的Token，并将它们组合成复合Token，从而在压缩KV缓存的同时保持原有缓存结构。
3. 实验表明，KVCompose在显著减少内存占用的同时，保持了模型精度，并且兼容现有推理流程，优于其他结构化和半结构化压缩方法。
